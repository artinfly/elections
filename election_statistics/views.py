...
from .models import DEG, METHODS, UIK, UIK19, UVZ, Employee
from .reports import (
    export_xlsx,
...
def _known_method(value: Any) -> str:
    """Валидирует значение способа голосования. Возвращает код или пустую строку."""
    return str(value) if value in {"deg", "uik", "uvz", "u19"} else ""
...
def _filtered(params: dict) -> QuerySet:
    ...
    # Поиск по словам (каждое слово ищется отдельно)
...
def _page_window(page: Any, size: int = 5) -> range:
    ...
    # Корректировка окна, если оно упирается в границы
    if end - start + 1 < size:
        if start == 1:
            end = min(total, size)
        else:
...
# ==============================================================================
# Загрузка данных (Upload handlers)
# ==============================================================================
...
def upload_base(request: HttpRequest) -> HttpResponse:
    """Обработчик POST-запроса с Excel-файлом для импорта базы сотрудников."""
    if not is_operator(request.user):
...
def upload_voting_choices(request: HttpRequest) -> HttpResponse:
    """Обработчик POST-запроса с Excel-файлом для импорта выборов способа голосования."""
    if not is_operator(request.user):
        return JsonResponse({"error": "нет прав на загрузку файлов"}, status=403)
...
def api_method(request: HttpRequest) -> JsonResponse:
    ...
    pk = _safe_int(data.get("id"))
    changed = Employee.objects.filter(pk=pk).update(
...
def api_mark(request: HttpRequest) -> JsonResponse:
    ...
    if person.method != MARK_FIELDS[field]:
        return JsonResponse(
            {"error": "способ голосования сотрудника не соответствует этому полю"},
            status=400,
        )
...
def _archive_response(request: HttpRequest, mode: str, prefix: str) -> HttpResponse:
    """Вспомогательная функция для генерации ZIP-архивов."""
